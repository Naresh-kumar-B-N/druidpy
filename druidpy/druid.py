...
class Druid(): 
	...
	def lookup_status(self,coordinator_url,tier='',lookup=''): # return the status of lookup(s)
		try:
			if len(lookup)>1:
	...
	def lookup_post(self,coordinator_url,tier,lookup,mysql_jdbc,db_user,db_password,key_column,val_column,poll_min):  # post the lookup into druid
		try:
			...
			vdata='{"'+tier+'":{''"'+lookup+'":{"version":"' +'v'+str(new_version)+'"'+',"lookupExtractorFactory":{' + '"type":"cachedNamespace",'+'"extractionNamespace":{'+'"type":"jdbc",'+'"connectorConfig":{'+'"createTables":true,'+'"connectURI":"'+''+mysql_jdbc+'",'+'"user":"'+db_user+'",'+'"password":"'+db_password+'"'+'},'+'"table":"'+lookup+'",'+'"keyColumn":"'+key_column+'",'+'"valueColumn":"'+val_column+'",'+'"pollPeriod": "PT'+str(poll_min)+'M"'+'},'+'"firstCacheTimeout":120000,"injective":true'+ '}}}}'
	...
	def ingest_task(self,overlord_url,ingestion_spec): #post the ingest task
		...
		try:
			if os.path.exists(ingestion_spec) == False:
				raise SystemExit("oops! Ingestion spec file does not exists , Please check!")
